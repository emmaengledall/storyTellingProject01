...
import {OrbitControls} from 'three/examples/jsm/controls/OrbitControls.js'; // Orbit control
import { EffectComposer } from 'three/examples/jsm/postprocessing/EffectComposer.js'; // Så vi kan loade effetker ind
import { RenderPass } from 'three/examples/jsm/postprocessing/RenderPass.js'; // Så vi kan rendere effetker
...
import Planet1 from './src/scenes/Planet1.js'; 
import SceneManager from './src/managers/SceneManager.js';


// MIT BASIC SETUP 
const scene = new THREE.Scene(); // Scenen 
const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000); // Kamera
camera.position.z = 5; // camera start position
const renderer = new THREE.WebGLRenderer(); // renderer 
renderer.setSize(window.innerWidth, window.innerHeight) // renderer indenfor canvas(størrelse)
...
// CONTROLS
const controls = new OrbitControls(camera, renderer.domElement); // orbit controls til kamera.
window.addEventListener('resize', () => { // eventlistner der lytter efter om canvas størrelse ændre sig.
  camera.aspect = window.innerWidth / window.innerHeight // hvis det ændre sig, så skal camera følge med. 
